...
> 提示：`ifconfig` 属于 net-tools 套件，在新系统中已被 `ip` 命令替代（iproute2）。建议新环境使用 `ip addr` / `ip link`。
...
```sh
ip addr show        # 查看 IP 地址（对应 ifconfig -a）
ip link show        # 查看网卡状态
ip addr add 192.168.1.2/24 dev eth0   # 配置 IP
ip link set eth0 up     # 启用网卡（对应 ifconfig eth0 up）
```
